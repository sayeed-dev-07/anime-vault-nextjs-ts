...
const MangaCard = ({ data }: { data: Manga }) => {
    ...
    return (
        <motion.div  viewport={{ once: true }} initial={{opacity:0, y:20}} whileInView={{opacity:1, y:0}} transition={{duration:0.2, type:'spring', delay:0.1}} className='bg-chatgpt-card border border-[#b3b3b3] mb-3 rounded-xl p-2  ease-linear will-change-auto duration-200 group shadow-md hover:shadow-xl  transition-all relative'>
            ...
            <div className='flex items-center justify-between px-2 mt-3 '>
                <motion.div  viewport={{ once: true }} initial={{x:-12, opacity:0}} whileInView={{x:0, opacity: 1}} transition={{duration: 0.3, type:'spring', delay:0.2}} className='flex gap-y-3 flex-col'>
            ...
            <motion.div  viewport={{ once: true }} variants={parent} initial="hidden" whileInView="show" className='flex items-center flex-wrap justify-center my-4 gap-2'>
                {
